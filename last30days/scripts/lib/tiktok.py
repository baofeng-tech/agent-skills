"""TikTok search for /last30days.

Preferred path: AISA-backed web discovery via site-constrained search.
Legacy compatibility path: third-party API search when an explicit legacy key
is still configured.
"""
...
def _hashtag_search(
    ...
    """Search TikTok by hashtag via the legacy compatibility backend.
...
def _profile_videos(
    ...
    """Fetch a TikTok creator's recent videos via the legacy backend.
...
def search_tiktok(
    ...
    """Search TikTok via the legacy compatibility API.
...
def fetch_captions(
    ...
    """Fetch transcripts for top N TikTok videos via the legacy backend.
