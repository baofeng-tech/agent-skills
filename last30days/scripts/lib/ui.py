...
More sources means better research, but it works fine as-is. Add `AISA_API_KEY` to unlock the hosted X, YouTube, web, and Polymarket path in one step. Reddit and HN can already contribute on their public routes. GitHub uses its official API path and may still need `GH_TOKEN` or `GITHUB_TOKEN` (or an existing `gh` login).
...
def show_diagnostic_banner(diag: dict):
    ...
    if IS_TTY:
        ...
        lines.append(f"{Colors.DIM}│{Colors.RESET} {Colors.BOLD}/last30days v1.0.0 - Source Status{Colors.RESET}                 {Colors.DIM}│{Colors.RESET}")
        ...
        if has_x:
            ...
            if x_backend == "aisa":
                label = "AISA proxy"
            elif x_backend == "bird" and username:
                label = f"Legacy Bird ({username})"
            elif x_backend:
                label = f"Legacy {str(x_backend).upper()}"
            else:
                label = "AISA proxy"
        ...
        lines.append("┌─────────────────────────────────────────────────────┐")
        lines.append("│ /last30days v1.0.0 - Source Status                 │")
        lines.append("│                                                     │")
        ...
        if has_x:
            if x_backend == "aisa":
                ...
                lines.append("│  ✅ X/Twitter — legacy backend available             │")
        else:
