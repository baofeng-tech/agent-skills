"""Instagram search for /last30days.

Preferred path: AISA-backed web discovery via site-constrained search.
Legacy compatibility path: third-party reel search when an explicit legacy key
is still configured.
"""
...
def _looks_like_aisa_key(token: str | None) -> bool:
...
def _user_reels(
    ...
    """Fetch an Instagram user's recent reels via the legacy backend.
...
def search_instagram(
    ...
    """Search Instagram Reels via the legacy compatibility API.
    ...
    if not _requests:
        ...
        try:
            ...
            _log(f"Legacy API error (urllib): {e}")
        ...
        try:
            ...
            _log(f"Legacy API error: {e}")
            return {"items": [], "error": f"{type(e).__name__}: {e}"}

    # Items are commonly returned in the 'reels' array on the legacy response
...
def fetch_captions(
    ...
    """Fetch transcripts for top N Instagram reels via the legacy backend.
...
def search_and_enrich(
    ...
    prefers the hosted AISA discovery path, and only falls back to the legacy
    API when an explicit compatibility key is supplied.
    ...
    Args:
        ...
        to_date: End date (YYYY-MM-DD)
        depth: 'quick', 'default', or 'deep'
        token: AISA key or legacy compatibility key
    ...
    # Step 1: Multi-query keyword search on the legacy path
