"""Legacy Bird X compatibility module.

The vendored Bird runtime has been removed from the default repo layout.
This module remains only so older compatibility branches can fail gracefully.
"""
...
# Path to the removed vendored wrapper location (kept only for compatibility checks)
...
# Module-level legacy X session cookies injected from config
_credentials: Dict[str, str] = {}


def set_credentials(auth_token: Optional[str], ct0: Optional[str]):
    """Inject legacy X session cookies from config for compatibility checks."""
    if auth_token:
...
def _has_process_credentials() -> bool:
    """Return True when legacy X session cookies are present in process env."""
...
def is_bird_installed() -> bool:
    """Bird is no longer bundled in the default repo layout."""
    return False
...
def install_bird() -> Tuple[bool, str]:
    """No-op compatibility hook for older setup code.

    Returns:
        Tuple of (success, message).
    """
    if not shutil.which("node"):
        return False, "Node.js is not installed, and the legacy Bird runtime is not bundled."
    return False, "Legacy Bird runtime is not bundled; use AISA_API_KEY instead."
...
def _run_bird_search(query: str, count: int, timeout: int) -> Dict[str, Any]:
    """Legacy Bird runtime is disabled; always fail gracefully."""
    del query, count, timeout
    return {"error": "Legacy Bird runtime is unavailable", "items": []}
...
def parse_bird_response(response: Dict[str, Any], query: str = "") -> List[Dict[str, Any]]:
    """Parse Bird response to match the normalized X item format.
    ...
    Returns:
        List of normalized item dicts matching the standard X parser shape.
    """
    items = []
