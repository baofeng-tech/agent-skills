...
def get_config() -> dict[str, Any]:
    ...
    # Legacy browser-cookie extraction is opt-in only.
    # AISA-first setups should not silently probe local browser state.
    if (config.get("FROM_BROWSER") or "").strip():
...
# ---------------------------------------------------------------------------
# Legacy browser cookie extraction
# ---------------------------------------------------------------------------
...
def extract_browser_credentials(config: dict[str, Any]) -> dict[str, str]:
    """Extract legacy auth cookies from local browsers.

    Browser probing is opt-in only. When FROM_BROWSER is unset or "off",
    no local browser state is read.

    Set ``FROM_BROWSER=firefox|safari|chrome|auto`` to explicitly enable
    extraction for compatibility-only legacy paths.
    """
...
def get_x_source_with_method(config: dict[str, Any]) -> tuple[str | None, str]:
    ...
    AISA is the preferred hosted path. Older xAI and explicit cookie-based
    routes remain available only as compatibility fallbacks.
    """
...
def get_reddit_source(config: dict[str, Any]) -> str | None:
    ...
    Returns: 'public' or 'scrapecreators'
    """
    del config
    return 'public'
...
def get_x_source(config: dict[str, Any]) -> str | None:
    ...
    Priority: explicit backend pin, then AISA, then legacy xAI, then legacy
    cookie-backed Bird compatibility.
    ...
    Returns:
        'aisa' if AISA_API_KEY is configured,
        'bird' if the legacy Bird compatibility layer is explicitly configured,
        'xai' if the legacy xAI compatibility path is configured,
...
def is_youtube_comments_available(config: dict[str, Any]) -> bool:
    """Check if legacy YouTube comment enrichment is explicitly enabled.

    This is not part of the default AISA-first YouTube path. It only turns on
    when a legacy compatibility key is configured and `youtube_comments` is
    explicitly requested in `INCLUDE_SOURCES`.
    """
...
def is_tiktok_available(config: dict[str, Any]) -> bool:
    ...
    AISA-hosted site search is the preferred path. Legacy third-party tokens
    remain supported only as an explicit compatibility fallback.
    """
    return bool(
        config.get('AISA_API_KEY')
        or config.get('SCRAPECREATORS_API_KEY')
        or config.get('APIFY_API_TOKEN')
    )


def get_tiktok_token(config: dict[str, Any]) -> str:
    """Get TikTok access token, preferring AISA over legacy providers."""
    return (
        config.get('AISA_API_KEY')
        or config.get('SCRAPECREATORS_API_KEY')
        or config.get('APIFY_API_TOKEN')
...
def is_threads_available(config: dict[str, Any]) -> bool:
    ...
    Threads is opt-in. Prefer the AISA-hosted site search path, with legacy
    third-party access only as an explicit fallback.
    """
    if not (config.get('AISA_API_KEY') or config.get('SCRAPECREATORS_API_KEY')):
        return False
...
def is_instagram_available(config: dict[str, Any]) -> bool:
    ...
    Prefer the AISA-hosted site search path, with legacy third-party access
    only as an explicit fallback.
    """
    return bool(config.get('AISA_API_KEY') or config.get('SCRAPECREATORS_API_KEY'))


def get_instagram_token(config: dict[str, Any]) -> str:
    """Get Instagram access token, preferring AISA over legacy providers."""
    return config.get('AISA_API_KEY') or config.get('SCRAPECREATORS_API_KEY') or ''
...
def get_x_source_status(config: dict[str, Any]) -> dict[str, Any]:
    ...
    Returns:
        ...
        bird_username, aisa_available, xai_available, can_install_bird
    """
    from . import bird_x

    bird_status = bird_x.get_bird_status()
    legacy_xai_available = bool(config.get('XAI_API_KEY'))
    ...
    return {
        ...
        "xai_available": legacy_xai_available,
...
def is_pinterest_available(config: dict[str, Any]) -> bool:
    ...
    Pinterest is opt-in. Prefer the AISA-hosted site search path, with legacy
    third-party access only as an explicit fallback.
    """
    if not (config.get('AISA_API_KEY') or config.get('SCRAPECREATORS_API_KEY')):
        return False
...
def get_pinterest_token(config: dict[str, Any]) -> str:
    """Get Pinterest access token, preferring AISA over legacy providers."""
    return config.get('AISA_API_KEY') or config.get('SCRAPECREATORS_API_KEY') or ''


# Xquik
def is_xquik_available(config: dict[str, Any]) -> bool:
    """Legacy Xquik source is no longer exposed in the AISA-first runtime."""
    del config
    return False
