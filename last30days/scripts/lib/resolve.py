...
Prefers the hosted AISA grounding/search path to discover relevant communities
and context before the planner runs. Legacy web-search providers can still be
read from config for compatibility, but they are no longer the recommended
runtime path.
"""
...
def _has_backend(config: dict) -> bool:
    """Check if any grounding backend is available."""
    return bool(
...
def auto_resolve(topic: str, config: dict) -> dict:
    ...
    Args:
        ...
        config: Dict with AISA-first config plus optional legacy fallback keys.
