...
def _normalize_provider_name(provider_name: str) -> str:
    return _ALIASED_PROVIDERS.get(provider_name, provider_name)
...
def warn_if_legacy_provider_alias(provider_name: str) -> None:
    ...
    effective = _normalize_provider_name(normalized)
    if normalized and normalized != effective:
        print(
            f"[Providers] LAST30DAYS_REASONING_PROVIDER={normalized!r} is a legacy compatibility alias; "
            f"routing through AISA as {effective!r}. Prefer 'auto' or 'aisa'.",
            file=sys.stderr,
        )
