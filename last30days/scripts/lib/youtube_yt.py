"""YouTube search and transcript extraction for the v3 pipeline.

The default search path is AISA-hosted. A few transcript/comment helpers remain
as legacy compatibility fallbacks for older environments.
"""
...
def is_ytdlp_installed() -> bool:
    """Legacy compatibility probe for the historical local downloader path."""
    return False
...
def _fetch_transcript_ytdlp(video_id: str, temp_dir: str) -> Optional[str]:
    """Legacy transcript helper using the historical local downloader path.
    ...
    # Historical downloader variants may save as .en.vtt or .en-orig.vtt
...
def search_and_transcribe(
    ...
    *,
    ...
    runtime path stays fully AISA-first.
...
# ---------------------------------------------------------------------------
# Legacy YouTube compatibility helpers
# ---------------------------------------------------------------------------
...
def _sc_headers(token: str) -> Dict[str, str]:
    """Build headers for the legacy compatibility API."""
    return {
        "x-api-key": token,
...
def enrich_with_comments(
    ...
    """Legacy opt-in comment enrichment for YouTube items.
...
def _fetch_video_comments(
    ...
    """Fetch comments for a single YouTube video via the legacy compatibility API.
...
def _sc_youtube_search(keyword: str, token: str) -> List[Dict[str, Any]]:
    ...
    if not _requests:
        try:
            ...
            _log(f"Legacy compatibility YouTube search error (urllib): {exc}")
            return []
    ...
    try:
        ...
        _log(f"Legacy compatibility YouTube search error: {exc}")
        return []


def _sc_fetch_transcript(video_id: str, token: str) -> Optional[str]:
    """Fetch transcript for a YouTube video via the legacy compatibility API.
    ...
    if not _requests:
        try:
            ...
            _log(f"Legacy transcript API error (urllib) for {video_id}: {exc}")
            return None
    else:
        try:
            resp = _requests.get(
            ...
            if resp.status_code != 200:
                _log(f"Legacy transcript API returned {resp.status_code} for {video_id}")
                return None
            data = resp.json()
        except Exception as exc:
            _log(f"Legacy transcript API error for {video_id}: {exc}")
            return None
