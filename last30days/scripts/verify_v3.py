...
def verify_unit() -> dict[str, str]:
    run_command([PYTHON, "-m", "pytest", "--capture=no", *CURATED_TESTS], timeout=600)
    run_command(
        [
            PYTHON,
            "-m",
...
def verify_smoke() -> list[dict[str, object]]:
    ...
    for provider, extra in SMOKE_CASES:
        ...
        start = time.time()
        print(
            f"[verify_v3] smoke start provider={provider} mode={'live' if has_live_hosted else 'mock'} args={' '.join(extra)}",
            file=sys.stderr,
            flush=True,
        )
        try:
            result = run_command(smoke_args, env=env, timeout=120)
